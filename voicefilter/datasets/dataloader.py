...
def create_dataloader(hp, args, train):
    def train_collate_fn(batch):
        ...
        mixed_path_list = list()

        for dvec_mel, target_mag, mixed_mag,target_path, mixed_path in batch:
            ...
            mixed_path_list.append(mixed_path)

        target_mag_list = torch.stack(target_mag_list, dim=0)
        mixed_mag_list = torch.stack(mixed_mag_list, dim=0)


        return dvec_list, target_mag_list, mixed_mag_list,target_path_list, mixed_path_list
...
class VFDataset(Dataset):
    def __init__(self, hp, args, train):
        def find_all(file_format):
            return sorted(glob.glob(os.path.join(self.data_dir, file_format)))[:200000]
        self.hp = hp
        self.args = args
        self.train = train
    ...
    def __getitem__(self, idx):
        ...
        if self.train: # need to be fast
            ...
            return dvec_mel, target_mag, mixed_mag, self.target_mag_list[idx], self.mixed_wav_list[idx]
        else:
            ...
            return dvec_mel, target_wav, mixed_wav, target_mag, mixed_mag, mixed_phase, self.target_mag_list[idx], self.target_wav_list[idx]
